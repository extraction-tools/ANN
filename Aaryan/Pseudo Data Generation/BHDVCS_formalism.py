########################### Modified on 07/08/2022 ####################
####### This file contains classes of formalisms    ###################
#######################################################################
...
class BKM02(object):
    ...
    def SetKinematics(self, kinematics):
        ...
        self.Gamma1 = self.x * self.y * self.y / self.ALP_INV / self.ALP_INV / self.ALP_INV / self.PI / 8. / self.QQ / self.QQ / np.sqrt( 1. + self.ee ) # factor in front of the cross section, eq. (22)
        self.s = 2. * self.M * self.k + self.M2
        self.tmin = - self.QQ * ( 2. * ( 1. - self.x ) * ( 1. - np.sqrt(1. + self.ee) ) + self.ee ) / ( 4. * self.x * ( 1. - self.x ) + self.ee ) # eq. (31)
        self.K2 = - ( self.t / self.QQ ) * ( 1. - self.x ) * ( 1. - self.y - self.y * self.y * self.ee / 4.) * ( 1. - self.tmin / self.t ) * ( np.sqrt(1. + self.ee) + ( ( 4. * self.x * ( 1. - self.x ) + self.ee ) / ( 4. * ( 1. - self.x ) ) ) * ( ( self.t - self.tmin ) / self.QQ )  ) # eq. (30)
    ...
    #K*D 4-vector product (phi-dependent)
        self.KD = - self.QQ / ( 2. * self.y * ( 1. + self.ee ) ) * ( 1. + 2. * np.sqrt(self.K2) * np.cos( self.PI - ( phi * self.RAD ) ) - self.t / self.QQ * ( 1. - self.x * ( 2. - self.y ) + self.y * self.ee / 2. ) + self.y * self.ee / 2.  ) #eq. (29)
    ...
    def BHUU(self, kinematics, phi, F1, F2) :
        ...
        self.c1_BH = 8. * np.sqrt(self.K2) * ( 2. - self.y ) * ( ( 4. * self.x * self.x * self.M2 / self.t - 2. * self.x - self.ee ) * ( F1 * F1 - F2 * F2 * self.t / ( 4. * self.M2 ) ) + 2. * self.x * self.x * ( 1. - ( 1. - 2. * self.x ) * self.t / self.QQ ) * ( F1 + F2 ) * ( F1 + F2 ) )
    ...
    def IUU(self, kinematics, phi, F1, F2, ReH, ReE, ReHtilde) :
        
        self.BHLeptonPropagators(kinematics, phi)
        
        self.A = - 8. * self.K2 * ( 2. - self.y ) * ( 2. - self.y ) * ( 2. - self.y ) / ( 1. - self.y ) - 8. * ( 2. - self.y ) * ( 1. - self.y ) * ( 2. - self.x ) * self.t / self.QQ - 8. * np.sqrt(self.K2) * ( 2. - 2. * self.y + self.y * self.y ) * np.cos( self.PI - (phi * self.RAD) )
        self.B = 8. * self.x * self.x * ( 2. - self.y ) * (1 - self.y ) / ( 2. - self.x ) * self.t / self.QQ
        self.C =  self.x / ( 2. - self.x ) * ( - 8. * self.K2 * ( 2. - self.y ) * ( 2. - self.y ) * ( 2. - self.y ) / ( 1. - self.y ) - 8. * np.sqrt(self.K2) * ( 2. - 2. * self.y + self.y * self.y ) * np.cos( self.PI - (phi * self.RAD) ) )
    ...
    def loss_MSE(self, kins, cffs, errs, f_true):
        ...
        #return np.mean(np.square((f_pred-f_true)/errs))
        return np.mean(np.square((f_pred-f_true)))
    ...
    def loss_MAE(self, kins, cffs, errs, f_true):
        ...
        xsiuu    = self.IUU(phi, kin1, kin2, kin3, kin4, F1, F2, ReH, ReE, ReHtilde)
        f_pred = xsbhuu + xsiuu +  c1fit
        return np.mean(np.abs((f_pred-f_true)/f_true))
    ...
    def curve_fit2(self, kins, ReH, ReE, ReHtilde, c0fit):
        ...
        xsiuu    = self.IUU(kinematics, phi, F1, F2, ReH, ReE, ReHtilde)
    ...
    def loss_chisq_v2(self, kins, cffs, errs, f_true):
        ...
        xsiuu    = self.IUU(phi, kin1, kin2, kin3, kin4, F1, F2, ReH, ReE, ReHtilde)
        f_pred = xsbhuu + xsiuu +  c1fit
        chisquare =  np.mean(np.square((f_pred-f_true)/errs))
        return np.abs(1-chisquare)
    ...
    def loss_play(self, kins, cffs, errs, f_true):
        ...
        f_pred = xsbhuu + xsiuu +  c1fit
        chisquare =  np.mean(np.square((f_pred-f_true)/errs))
        mse = np.mean(np.square((f_pred-f_true)))
