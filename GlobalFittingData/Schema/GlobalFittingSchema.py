...
class GlobalFittingSchema:
    ...
    def getStatistics(self, architecture: str, run: str, cff: str):
        ...
        if (cff == 'ReH'):
            return [stat for stat in self.data[architecture][run][1][0]]
        elif (cff == 'ReE'):
            return [stat for stat in self.data[architecture][run][1][1]]
        elif (cff == 'ReHtilde'):
            return [stat for stat in self.data[architecture][run][1][2]]
        else:
    ...
    def getStatistic(self, architecture: str, run: str, cff: str, statistic: str):
        ...
        if (cff == 'ReH'):
            statistics = self.data[architecture][run][1][0]
        elif (cff == 'ReE'):
            statistics = self.data[architecture][run][1][1]
        elif (cff == 'ReHtilde'):
            statistics = self.data[architecture][run][1][2]
        else:
            raise Exception("Invalid CFF")
    ...
    def setStatistic(self, architecture: str, run: str, cff: str, statistic: str, value):
        ...
        if (cff == 'ReH'):
            self.data[architecture][run][1][0][statistic] = value
        elif (cff == 'ReE'):
            self.data[architecture][run][1][1][statistic] = value
        elif (cff == 'ReHtilde'):
            self.data[architecture][run][1][2][statistic] = value
        else:
            raise Exception("Invalid CFF")
