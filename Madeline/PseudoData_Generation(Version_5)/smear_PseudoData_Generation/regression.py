...
dataset = [0.02, 0.04, 0.06, 0.08, 0.1, 0.12, 0.14, 0.16, 0.18, 0.2]
...
#     axs[itr].text(53, 0.075, 'MAE='+str(test_results['dnn_model']),verticalalignment='bottom', horizontalalignment='left',color='green', fontsize=10)
    axs[itr].text(50, 0.018, 'MAE='+str('%s' % float('%.4g' % test_results['dnn_model'])),verticalalignment='bottom', horizontalalignment='left',color='green', fontsize=10)
    '%s' % float('%.1g' % 1234)
    
    axs[itr].text(50, 0.023, 'smear err ='+str(data),verticalalignment='bottom', horizontalalignment='left',color='green', fontsize=10)
    axs[itr].set_xlabel('Epoch')
    axs[itr].set_ylabel('Error [F]')

# axs[0].plot(history.history['loss'], label='loss')
# axs[0].plot(history.history['val_loss'], label='val_loss')
axs[0].set_title('Varied Smear Error')
