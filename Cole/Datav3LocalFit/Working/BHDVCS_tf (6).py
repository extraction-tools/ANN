...
class TotalFLayer(tf.keras.layers.Layer):
    ...
    def __init__(self):
        super(TotalFLayer, self).__init__(dtype='float64')
        self.F=BHDVCStf()
...
class BHDVCStf(tf.Module):
    ...
    def SetKinematics(self, _QQ, _x, _t, _k):
        ...
        self.Gamma1 = self.x * self.y * self.y / self.ALP_INV / self.ALP_INV / self.ALP_INV / self.PI / 8. / self.QQ / self.QQ / tf.sqrt( 1. + self.ee ) # factor in front of the cross section, eq. (22)
        self.s = 2. * self.M * self.k + self.M2
        self.tmin = - self.QQ * ( 2. * ( 1. - self.x ) * ( 1. - tf.sqrt(1. + self.ee) ) + self.ee ) / ( 4. * self.x * ( 1. - self.x ) + self.ee ) # eq. (31)
        self.K2 = - ( self.t / self.QQ ) * ( 1. - self.x ) * ( 1. - self.y - self.y * self.y * self.ee / 4.) * ( 1. - self.tmin / self.t ) * ( tf.sqrt(1. + self.ee) + ( ( 4. * self.x * ( 1. - self.x ) + self.ee ) / ( 4. * ( 1. - self.x ) ) ) * ( ( self.t - self.tmin ) / self.QQ )  ) # eq. (30)
	...
	#K*D 4-vector product (phi-dependent)
        self.KD = - self.QQ / ( 2. * self.y * ( 1. + self.ee ) ) * ( 1. + 2. * tf.sqrt(self.K2) * tf.cos( self.PI - ( phi * self.RAD ) ) - self.t / self.QQ * ( 1. - self.x * ( 2. - self.y ) + self.y * self.ee / 2. ) + self.y * self.ee / 2.  ) #eq. (29)
    ...
    def BHUU(self, phi, _QQ, _x, _t, _k, F1, F2) :
        ...
        self.c1_BH = 8. * tf.sqrt(self.K2) * ( 2. - self.y ) * ( ( 4. * self.x * self.x * self.M2 / self.t - 2. * self.x - self.ee ) * ( F1 * F1 - F2 * F2 * self.t / ( 4. * self.M2 ) ) + 2. * self.x * self.x * ( 1. - ( 1. - 2. * self.x ) * self.t / self.QQ ) * ( F1 + F2 ) * ( F1 + F2 ) )
  	...
  	#BH squared amplitude eq (25) divided by e^6
        self.Amp2_BH = 1. / ( self.x * self.x * self.y * self.y * ( 1. + self.ee ) * ( 1. + self.ee ) * self.t * self.P1 * self.P2 ) * ( self.c0_BH + self.c1_BH * tf.cos( self.PI - (phi * self.RAD) ) + self.c2_BH * tf.cos( 2. * ( self.PI - ( phi * self.RAD ) ) )  )
    ...
    def IUU(self, phi, _QQ, _x, _t, _k, F1, F2, ReH, ReE, ReHtilde) :
        ...
        self.A = - 8. * self.K2 * ( 2. - self.y ) * ( 2. - self.y ) * ( 2. - self.y ) / ( 1. - self.y ) - 8. * ( 2. - self.y ) * ( 1. - self.y ) * ( 2. - self.x ) * self.t / self.QQ - 8. * tf.sqrt(self.K2) * ( 2. - 2. * self.y + self.y * self.y ) * tf.cos( self.PI - (phi * self.RAD) )
        self.B = 8. * self.x * self.x * ( 2. - self.y ) * (1 - self.y ) / ( 2. - self.x ) * self.t / self.QQ
        self.C =  self.x / ( 2. - self.x ) * ( - 8. * self.K2 * ( 2. - self.y ) * ( 2. - self.y ) * ( 2. - self.y ) / ( 1. - self.y ) - 8. * tf.sqrt(self.K2) * ( 2. - 2. * self.y + self.y * self.y ) * tf.cos( self.PI - (phi * self.RAD) ) )
    ...
    def curve_fit_array(self, kins, cffs):
      ...
      phi, kin1, kin2, kin3, kin4, F1, F2 = tf.split(kins, num_or_size_splits=7, axis=1)
    ...
    def loss_chisq_v2(self, kins, cffs, errs, f_true):
        ...
        xsiuu    = self.IUU(phi, kin1, kin2, kin3, kin4, F1, F2, ReH, ReE, ReHtilde)
        f_pred = xsbhuu + xsiuu +  c1fit 
        chisquare =  tf.mean(tf.square((f_pred-f_true)/errs))
        return tf.abs(1-chisquare)
    ...
    def loss_play(self, kins, cffs, errs, f_true):
        ...
        f_pred = xsbhuu + xsiuu +  c1fit 
        chisquare =  tf.mean(tf.square((f_pred-f_true)/errs))
        mse = tf.mean(tf.square((f_pred-f_true)))
