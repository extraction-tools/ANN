...
class DVCSFIT(object):
    ...
    def plot_linear(self, xaxis, mparm, cparm):
        return (mparm * xaxis + cparm)
    ...
    def const_term(self, kins): # this is the proportional constant for reduced F
        phi, kin1, kin2, kin3, kin4, F1, F2 = kins
        kinematics = [kin1, kin2, kin3, kin4]
        self.SetKinematics(kinematics)
        self.BHLeptonPropagators(kinematics, phi)
        self.ABC_UU_I_10(kinematics, phi, "t2");
        c_term = ( self.x * self.y * self.y * self.y * self.t * self.P1 * self.P2 ) / (self.B_U_I * self.Gamma * self.GeV2nb * 1000)
        return c_term

    def AperB(self, kins): # this is the x axis for Linear Fit
        phi, kin1, kin2, kin3, kin4, F1, F2 = kins
        kinematics = [kin1, kin2, kin3, kin4]
        self.SetKinematics(kinematics)
        self.BHLeptonPropagators(kinematics, phi)
        self.ABC_UU_I_10(kinematics, phi, "t2");
        x_axis = self.A_U_I / self.B_U_I
        return x_axis

    def BperC(self, kins): # this is the x axis for Linear Fit
        phi, kin1, kin2, kin3, kin4, F1, F2 = kins
        kinematics = [kin1, kin2, kin3, kin4]
        self.SetKinematics(kinematics)
        self.BHLeptonPropagators(kinematics, phi)
        self.ABC_UU_I_10(kinematics, phi, "t2");
        ratio = self.B_U_I / self.C_U_I
        return ratio
