...
dats = pd.read_csv('pseudo_BKM10_hallA_t2_nosmear.csv')
...
df = pd.read_csv("pseudo_BKM10_hallA_t2_nosmear.csv")
def F2VsPhi(dataframe,SetNum,xdat,cffs):
    ...
    plt.errorbar(np.cos(np.radians(temp_phi[mask])),TempFvals[mask],TempFvals_sigma[mask],fmt='.',color='blue',label="Data")
    #plt.xlim(0,368)
...
for ii in range(5):
 ...
 #m = Minuit(LSQ, ReH = gReH[a+6], ReE = gReE[a+6], ReHtilde = gReHTilde[a+6], c0fit = gc0fit[a+6], c1fit = gc1fit[a+6]) # This is for testing the truth values
 ...
 #m.errors = (0.0000000001, 0.0000000001,0.0000000001,0.0000000001,0.0000000001) #initial step size. Also not sure if this is useful
 #m.print_level = 0 # remove noisy warning if you want
 ...
 #F2VsPhi(df,ii+1,xdat,cffs) # this plot F
 F2VsPhi_r(df,ii+1,xdat,cffs) # this plot reduced F or the interference term
 plt.clf()
 ## The following lines are for testing and debugging  
 #print(f(xdat, cffs)) 
 #print(func_fit(xdat, ReH_fit, ReE_fit, ReHtilde_fit, c0fit_fit, c1fit_fit))
 #print(F[a:b])
