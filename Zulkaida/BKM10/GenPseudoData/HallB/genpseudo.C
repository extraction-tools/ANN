...
void genpseudo()
{
   ...
   // Kinematic settings of the JLab Hall B data
    ...
    for (int i=0; i<NumOfSets; i++)
     {
      xB_A[i] = temp[NumOfDataPts*7*i + 0];
      QQ_A[i] = temp[NumOfDataPts*7*i + 1];
      t_A[i] = -1.*temp[NumOfDataPts*7*i + 2];
      F1_A[i] = ff->ffF1_K(t_A[i]);
      F2_A[i] = ff->ffF2_K(t_A[i]);
      for (int j=0; j<NumOfDataPts; j++)
       {
        phi_A[i][j] = temp[NumOfDataPts*7*i + 7*j + 3];
	F_data[i][j] = temp[NumOfDataPts*7*i + 7*j + 4];
        stat_err[i][j] = temp[NumOfDataPts*7*i + 7*j + 5];
	...
	myfile.open ("pseudo_BKM10_hallB_t3.csv");
	myfile<<"#Set,index,k,QQ,x_b,t,phi_x,F,sigmaF,varF,F1,F2,gReH,gReE,gReHTilde"<<endl;
...
Double_t TotalUUXS(Double_t *angle, Double_t *par)	// Total Cross Section - user defined function
{
	...
	TComplex CFFs[4] = {_H, _E, _HTilde, _ETilde};
        Double_t xsiuu = bhdvcs ->I_UU_10(kinset, _phi, _F1, _F2, CFFs, "t3"); // Interference
	Double_t xsdvcs = bhdvcs ->DVCS_UU_10(kinset, _phi, CFFs, "t3"); // dvcs
