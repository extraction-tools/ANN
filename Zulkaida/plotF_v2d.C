 ...
 for (int i=0; i<10; i++)
  {
   ...
   for (int j=0; j<1000; j++)
    {
      bin_zul_ReH[i]->Fill(temp2[10*3*j+3*i+0]);
    }
 ...
 for (int i=0; i<10; i++)
  {
   ...
   for (int j=0; j<1000; j++)
    {
      bin_zul_ReE[i]->Fill(temp2[10*3*j+3*i+1]);
    }
 ...
 for (int i=0; i<10; i++)
  {
   ...
   for (int j=0; j<1000; j++)
    {
      bin_zul_ReHTilde[i]->Fill(temp2[10*3*j+3*i+2]);
    }
  ...
  for (int j=0; j<45; j++)
      {
       ...
       //loss_data[i] = loss_data[i] + (F[i][j] - F_fit_data[i][j])*(F[i][j] - F_fit_data[i][j]);
  ...
  for (int j=0; j<45; j++)
      {
       ...
       //loss_truth[i] = loss_truth[i] + (F[i][j] - F_fit_truth[i][j])*(F[i][j] - F_fit_truth[i][j]);
...
//
  ...
  sprintf(name4,"plotv2d_result14_canvas%i_.png",i);
