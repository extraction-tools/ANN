...
by_set = []
kmeansInput = np.array([qq, xb, t, k])
kmeans = KMeans(n_clusters=70, random_state=0).fit(kmeansInput.reshape(-1,1)) 
...
for ii in range(15): # set how many sets to process
 ...
 #net = RBFNet(4, 4, centers, False) # untrain/reset network # untrain/reset network
 net = RBFNet2Layer(4, 4, centers1, centers2, True) # untrain/reset network # untrain/reset network
