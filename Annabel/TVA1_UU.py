...
class TVA1_UU(tf.Module):
    ...
    def SetKinematics(self, _QQ, _x, _t, _k):
        ...
        self.e = (1 - self.y - (self.y * self.y * (self.gg / 4.))) / (
                    1. - self.y + (self.y * self.y / 2.) + (self.y * self.y * (self.gg / 4.)))  # epsilon eq. (32)
        # Skewness parameter
        self.xi = 1. * self.x * ((1. + self.t / (2. * self.QQ)) / (
                    2. - self.x + self.x * self.t / self.QQ))  # skewness parameter eq. (12b) dnote: there is a minus sign on the write up that shouldn't be there
        # Minimum t value
        self.tmin = -(self.QQ * (1. - tf.sqrt(1. + self.gg) + self.gg / 2.)) / (
                    self.x * (1. - tf.sqrt(1. + self.gg) + self.gg / (2. * self.x)))  # minimum t eq. (29)
        # Final Lepton energy
        ...
        self.cth = -1. / tf.sqrt(1. + self.gg) * (1. + self.gg / 2. * (1. + self.t / self.QQ) / (
                    1. + self.x * self.t / self.QQ))  # This is tf.cos(theta) eq. (26)
        self.theta = tf.acos(self.cth)  # theta angle
        # print('Theta: ', self.theta)
        # Lepton Angle Kinematics of initial lepton
        self.sthl = tf.sqrt(self.gg) / tf.sqrt(1. + self.gg) * (
            tf.sqrt(1. - self.y - self.y * self.y * self.gg / 4.))  # sin(theta_l) from eq. (22a)
        ...
        self.Gamma = 1. / self.ALP_INV / self.ALP_INV / self.ALP_INV / self.PI / self.PI / 16. / (self.s - self.M2) / (
                    self.s - self.M2) / tf.sqrt(1. + self.gg) / self.x
    ...
    def Set4VectorsPhiDep(self, phi):
        ...
        self.QP.SetPxPyPzE(self.qp * tf.sin(self.theta) * tf.cos(phi * self.RAD),
                           self.qp * tf.sin(self.theta) * tf.sin(phi * self.RAD), self.qp * tf.cos(self.theta), self.qp)
    ...
    def Set4VectorProducts(self, phi):
        ...
        # Expressions that appear in interference calculations
    ...
    def GetBHUUxs(self, phi, F1, F2):
        ...
        self.AUUBH = ((8. * self.M2) / (self.t * self.kqp * self.kpqp)) * (
                    (4. * self.tau * (self.kP * self.kP + self.kpP * self.kpP)) - (
                        (self.tau + 1.) * (self.kd * self.kd + self.kpd * self.kpd)))
    ...
    def GetIUUxs(self, phi, F1, F2, ReH, ReE, ReHtilde):
        ...
        self.AUUI = -4. * tf.cos(phi * self.RAD) * (self.Dplus * (
                    (self.kqp_T - 2. * self.kk_T - 2. * self.kqp) * self.kpP + (
                        2. * self.kpqp - 2. * self.kkp_T - self.kpqp_T) * self.kP + self.kpqp * self.kP_T + self.kqp * self.kpP_T - 2. * self.kkp * self.kP_T) -
                                                    self.Dminus * ((
                                                                               2. * self.kkp - self.kpqp_T - self.kkp_T) * self.Pqp + 2. * self.kkp * self.qpP_T - self.kpqp * self.kP_T - self.kqp * self.kpP_T));

        self.BUUI = -2. * self.xi * tf.cos(phi * self.RAD) * (self.Dplus * (
                    (self.kqp_T - 2. * self.kk_T - 2. * self.kqp) * self.kpd + (
                        2. * self.kpqp - 2. * self.kkp_T - self.kpqp_T) * self.kd + self.kpqp * self.kd_T + self.kqp * self.kpd_T - 2. * self.kkp * self.kd_T) -
                                                              self.Dminus * ((
                                                                                         2. * self.kkp - self.kpqp_T - self.kkp_T) * self.qpd + 2. * self.kkp * self.qpd_T - self.kpqp * self.kd_T - self.kqp * self.kpd_T));

        self.CUUI = -2. * tf.cos(phi * self.RAD) * (self.Dplus * (
                    2. * self.kkp * self.kd_T - self.kpqp * self.kd_T - self.kqp * self.kpd_T + 4. * self.xi * self.kkp * self.kP_T - 2. * self.xi * self.kpqp * self.kP_T - 2. * self.xi * self.kqp * self.kpP_T) -
                                                    self.Dminus * (
                                                                self.kkp * self.qpd_T - self.kpqp * self.kd_T - self.kqp * self.kpd_T + 2. * self.xi * self.kkp * self.qpP_T - 2. * self.xi * self.kpqp * self.kP_T - 2. * self.xi * self.kqp * self.kpP_T));
        ...
        # Unpolarized Coefficients multiplied by the Form Factors
        self.iAUU = (self.Gamma / (tf.abs(self.t) * self.QQ)) * self.con_AUUI * (F1 * ReH + self.tau * F2 * ReE);
        self.iBUU = (self.Gamma / (tf.abs(self.t) * self.QQ)) * self.con_BUUI * (F1 + F2) * (ReH + ReE);
        self.iCUU = (self.Gamma / (tf.abs(self.t) * self.QQ)) * self.con_CUUI * (F1 + F2) * ReHtilde;
    ...
    def ABcon(self, phi, F1, F2):
        ...
        self.AUUI = -4.0 / (self.kqp * self.kpqp) * ((self.QQ + self.t) * (
                    2.0 * (self.kP + self.kpP) * self.kk_T + (self.Pq * self.kqp_T) + 2. * (
                        self.kpP * self.kqp) - 2. * (
                                self.kP * self.kpqp) + self.kpqp * self.kP_T + self.kqp * self.kpP_T - 2. * self.kkp * self.kP_T)
                                                     + (self.QQ - self.t + 4. * self.kd) * (self.Pqp * (
                            self.kkp_T + self.kqp_T - 2. * self.kkp) + 2. * self.kkp * self.qpP_T - self.kpqp * self.kP_T - self.kqp * self.kpP_T))

        self.BUUI = 2.0 * self.xi / (self.kqp * self.kpqp) * ((self.QQ + self.t) * (
                    2. * self.kk_T * (self.kd + self.kpd) + self.kqp_T * (
                        self.qd - self.kqp - self.kpqp + 2. * self.kkp) + 2. * self.kqp * self.kpd - 2. * self.kpqp * self.kd) +
                                                              (self.QQ - self.t + 4. * self.kd) * ((
                                                                                                               self.kk_T - 2. * self.kkp) * self.qpd - self.kkp * self.dd_T - 2. * self.kd_T * self.kqp))
    ...
    def TotalUUXS(self, x, ReH, ReE,
                  ReHtilde):  # originally named TotalUUXS_curve_fit, but original TotalUUXS is unnecessary
        """
        params:
