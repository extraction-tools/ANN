...
class TBHDVCS(object):
    ...
    def IUU(self, phi, _QQ, _x, _t, _k, F1, F2, ReH, ReE, ReHtilde, twist) :
        ...
        self.ABC_UU_I_10(phi, _QQ, _x, _t, _k, twist)
    ...
    def ABC_UU_I_10(self, phi, _QQ, _x, _t, _k, twist) : #Get A_UU_I, B_UU_I and C_UU_I interference coefficients BKM10
        #self.SetKinematics(kinematics)
        self.SetKinematics(_QQ, _x, _t, _k)
    ...
    def loss_MAE_2(self, kins, cffs, errs, f_true):
        phi, kin1, kin2, kin3, kin4, F1, F2 = kins
        ...
        xsiuu    = self.IUU(phi, kin1, kin2, kin3, kin4, F1, F2, ReH, ReE, ReHtilde,"t2")
        f_pred = xsbhuu + xsiuu +  c1fit
        return torch.mean(torch.abs((f_pred-f_true)/f_pred))

    def loss_MAE_custom(self, kins, cffs, errs, f_true):
        ReH, ReE, ReHtilde, c0fit = cffs #output of network
        phi, kin1, kin2, kin3, kin4, F1, F2 = kins
        kinematics = [kin1, kin2, kin3, kin4]
        self.SetKinematics(kinematics)
        ...
        xsiuu    = self.IUU(kinematics, phi, F1, F2, ReH, ReE, ReHtilde, "t2")
        f_pred = xsbhuu + xsiuu + c0fit
        return torch.mean(torch.abs((f_pred-f_true)/(f_true - xsbhuu)))

    def loss_log_mae(self, kins, cffs, errs, f_true):
        ReH, ReE, ReHtilde, c0fit = cffs #output of network
        phi, kin1, kin2, kin3, kin4, F1, F2 = kins
        kinematics = [kin1, kin2, kin3, kin4]
        self.SetKinematics(kinematics)
        ...
        xsiuu    = self.IUU(kinematics, phi, F1, F2, ReH, ReE, ReHtilde, "t2")
        f_pred = xsbhuu + xsiuu + c0fit
        if torch.mean(c0fit) > 0.00001:
           return torch.mean(torch.abs((torch.log(f_pred)-torch.log(f_true))/torch.log(f_true)))
        else:
           return 100.*torch.mean(torch.abs((f_pred-f_true)/f_true))

    def loss_log_mae_custom(self, kins, cffs, errs, f_true):
        ReH, ReE, ReHtilde, c0fit = cffs #output of network
        phi, kin1, kin2, kin3, kin4, F1, F2 = kins
        kinematics = [kin1, kin2, kin3, kin4]
        self.SetKinematics(kinematics)
        ...
        f_pred = xsbhuu + xsiuu + c0fit
        if torch.mean(c0fit) > 0.00001 :
           return torch.mean(torch.abs((torch.log(torch.abs(f_pred-xsbhuu))-torch.log(f_true-xsbhuu))/torch.log(f_true-xsbhuu)))
        else:
           return 100.*torch.mean(torch.abs((f_pred-f_true)/(f_true-xsbhuu)))

    def loss_log_mse(self, kins, cffs, errs, f_true):
        ReH, ReE, ReHtilde, c0fit = cffs #output of network
        phi, kin1, kin2, kin3, kin4, F1, F2 = kins
        kinematics = [kin1, kin2, kin3, kin4]
        self.SetKinematics(kinematics)
        ...
        f_pred = xsbhuu + xsiuu + c0fit
        if torch.mean(c0fit) > 0.00001:
           return torch.mean(torch.square((torch.log(f_pred)-torch.log(f_true))))
        else:
           return torch.mean(torch.square((f_pred-f_true)))
    ...
    def loss_chisq_custom(self, kins, cffs, errs, f_true):
        phi, kin1, kin2, kin3, kin4, F1, F2 = kins
        ReH, ReE, ReHtilde, c0fit = cffs #output of network
        kinematics = [kin1, kin2, kin3, kin4]
        self.SetKinematics(kinematics)
        ...
        xsiuu    = self.IUU(kinematics, phi, F1, F2, ReH, ReE, ReHtilde, "t2")
        f_pred = xsbhuu + xsiuu +  c0fit
        errs = errs * (f_pred - xsbhuu)/f_pred
       # if torch.abs(xsiuu[11]) < 0.0005:
        #   return torch.mean(torch.abs((f_pred-f_true)))
        #else:
        #   return 100000000.*torch.mean(torch.abs((f_pred-f_true)))
        return torch.sum(torch.abs(f_true - f_pred))
        

    def loss_log_chisq(self, kins, cffs, errs, f_true):
        ReH, ReE, ReHtilde, c0fit = cffs #output of network
        phi, kin1, kin2, kin3, kin4, F1, F2 = kins
        kinematics = [kin1, kin2, kin3, kin4]
        self.SetKinematics(kinematics)
        ...
        xsiuu    = self.IUU(kinematics, phi, F1, F2, ReH, ReE, ReHtilde, "t2")
        f_pred = xsbhuu + xsiuu + c0fit
        errs = errs * torch.abs(torch.log(f_pred)/f_pred)
        if torch.mean(c0fit) > 0.00001:
           return torch.mean(torch.abs((torch.log(f_pred)-torch.log(f_true))/errs))
        else:
           return 100.*torch.mean(torch.abs((f_pred-f_true)/f_true))
##new functions

    def loss_chisq_sum(self, kins, cffs, errs, f_true):
        phi, kin1, kin2, kin3, kin4, F1, F2 = kins
        ReH, ReE, ReHtilde, c0fit = cffs #output of network
        self.SetKinematics(kin1, kin2, kin3, kin4)
        xsbhuu   = self.BHUU(phi, kin1, kin2, kin3, kin4, F1, F2)
        xsiuu    = self.IUU(phi, kin1, kin2, kin3, kin4, F1, F2, ReH, ReE, ReHtilde, "t2")
        f_pred = xsbhuu + xsiuu +  c0fit
        return torch.sum(torch.square((f_pred-f_true)/errs))

    def loss_mae_sum(self, kins, cffs, errs, f_true):
        phi, kin1, kin2, kin3, kin4, F1, F2 = kins
        ReH, ReE, ReHtilde, c0fit = cffs #output of network
        self.SetKinematics(kin1, kin2, kin3, kin4)
        xsbhuu   = self.BHUU(phi, kin1, kin2, kin3, kin4, F1, F2)
        xsiuu    = self.IUU(phi, kin1, kin2, kin3, kin4, F1, F2, ReH, ReE, ReHtilde, "t2")
        f_pred = xsbhuu + xsiuu +  c0fit
        return torch.sum(torch.abs((f_pred-f_true)/f_true))

    def loss_chisq_sum_mod(self, kins, cffs, errs, f_true):
        phi, kin1, kin2, kin3, kin4, F1, F2 = kins
        ReH, ReE, ReHtilde, c0fit = cffs #output of network
        self.SetKinematics(kin1, kin2, kin3, kin4)
        xsbhuu   = self.BHUU(phi, kin1, kin2, kin3, kin4, F1, F2)
        xsiuu    = self.IUU(phi, kin1, kin2, kin3, kin4, F1, F2, ReH, ReE, ReHtilde, "t2")
        f_pred = xsbhuu + xsiuu +  torch.pow(10,c0fit)
        return torch.sum(torch.square((f_pred-f_true)/errs))

    def loss_chisq_sum_mod_v2(self, kins, cffs, errs, f_true):
        phi, kin1, kin2, kin3, kin4, F1, F2 = kins
        ReH, ReE, ReHtilde, c0fit = cffs #output of network
        self.SetKinematics(kin1, kin2, kin3, kin4)
        xsbhuu   = self.BHUU(phi, kin1, kin2, kin3, kin4, F1, F2)
        xsiuu    = self.IUU(phi, kin1, kin2, kin3, kin4, F1, F2, ReH, ReE, ReHtilde, "t2")
        f_pred = xsbhuu + xsiuu +  torch.exp(c0fit)
        return torch.sum(torch.square((f_pred-f_true)/errs))

    def loss_chisq_mean(self, kins, cffs, errs, f_true):
        phi, kin1, kin2, kin3, kin4, F1, F2 = kins
        ReH, ReE, ReHtilde, c0fit = cffs #output of network
        self.SetKinematics(kin1, kin2, kin3, kin4)
        xsbhuu   = self.BHUU(phi, kin1, kin2, kin3, kin4, F1, F2)
    ...
    def loss_chisq_mean_mod(self, kins, cffs, errs, f_true):
        phi, kin1, kin2, kin3, kin4, F1, F2 = kins
        ReH, ReE, ReHtilde, c0fit = cffs #output of network
        self.SetKinematics(kin1, kin2, kin3, kin4)
        xsbhuu   = self.BHUU(phi, kin1, kin2, kin3, kin4, F1, F2)
    ...
    def loss_chisq_mean_mod_v2(self, kins, cffs, errs, f_true):
        phi, kin1, kin2, kin3, kin4, F1, F2 = kins
        ReH, ReE, ReHtilde, c0fit = cffs #output of network
        self.SetKinematics(kin1, kin2, kin3, kin4)
        xsbhuu   = self.BHUU(phi, kin1, kin2, kin3, kin4, F1, F2)
    ...
    def getF_mod_v2(self, kins, cffs):
        phi, kin1, kin2, kin3, kin4, F1, F2 = kins
        ReH, ReE, ReHtilde, c0fit = cffs #output of network
        self.SetKinematics(kin1, kin2, kin3, kin4)
        xsbhuu   = self.BHUU(phi, kin1, kin2, kin3, kin4, F1, F2)
    ...
    def Generate_KM(self, kins, cffs):
        phi, kin1, kin2, kin3, kin4, F1, F2 = kins
        ReH, ReE, ReHtilde, c1fit = cffs #output of network
        self.SetKinematics(kin1, kin2, kin3, kin4)
        xsbhuu   = self.BHUU(phi, kin1, kin2, kin3, kin4, F1, F2)
