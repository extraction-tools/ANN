...
class ComparisonPlot:
    ...
    def find_farthest_values(values, true_value):
        # Calculate the absolute differences between each value and the true value
        differences = np.abs(values - true_value)
        
        # Find the index of the maximum and minimum differences
        max_index = np.argmax(differences)
        min_index = np.argmin(differences)
        
        # Get the maximum and minimum values based on the index
    ...
    def plot(self):
        if self.folder_path is not None:
            ...
            df_mean = pd.DataFrame(array_mean, columns=dfs[0].columns, index=dfs[0].index + 1)
            df_stdev = pd.DataFrame(array_stdev, columns=dfs[0].columns, index=dfs[0].index + 1)
            ...
            #repeat process for min and max
            #array_min = np.min(array_3d, axis=-1)
            #array_max = np.max(array_3d, axis=-1)
            #df_min = pd.DataFrame(array_min, columns=dfs[0].columns, index=dfs[0].index + 1)
            #df_max = pd.DataFrame(array_max, columns=dfs[0].columns, index=dfs[0].index + 1)
            #df_min.columns = ['ReH', 'ReE', 'ReHTilde', 'dvcs']
            #df_max.columns = ['ReH', 'ReE', 'ReHTilde', 'dvcs']
        ...
        # only using cffs
        result_array = result.iloc[:, 12:16].values
        
        # Create an empty array to store the differences
        array_3d_differences = np.empty_like(array_3d)
        
        # Iterate over the arrays within `array_3d`
        for i in range(array_3d.shape[-1]):
            # Calculate the differences between the current array and `result_array`
            array_3d_differences[..., i] = np.abs(array_3d[..., i] - result_array)
        
        # min max values from array differences
        min_values = np.min(array_3d_differences, axis=-1)
        max_values = np.max(array_3d_differences, axis=-1)

        # Create DataFrames from the minimum and maximum values arrays
        df_min = pd.DataFrame(min_values, columns=result.columns[12:16], index=dfs[0].index)
        df_max = pd.DataFrame(max_values, columns=result.columns[12:16], index=dfs[0].index)
        
        # Print the resulting DataFrames
        #print(df_min)
        #print(df_max)
        # Print the resulting differences array
        
        #result_array = result.values[..., np.newaxis]
        #print(array_3d - result_array)   
        #array_max, array_min = self.find_farthest_values(array_3d, result)
        #print(array_max)
        #print(array_min)
        ...
        if self.folder_path is not None:
            ...
            NPA = (df_mean[self.cff] - df_min[self.cff]) / (df_max[self.cff] - df_min[self.cff])
            NPA = np.sum(NPA) / len(NPA)
            #NPA = (1 - np.abs(NPA)) * 100
            #input Normalized Perecntage Accuracy here
            print(f"Rlative Percentage Accuracy: {np.abs(NPA * 100)}%")

            diff = np.abs(df_filtered[self.cff] - result_filtered[self.cff])
            within_std = np.sum(diff <= y_err_filtered)
            accuracy = (within_std / len(diff)) * 100
