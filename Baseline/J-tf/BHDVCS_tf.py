...
class TotalFLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        return self.f.curve_fit(inputs[:, 0:6])
...
class DvcsData(object):
    ...
    def getSet(self, setNum, startIndex, endIndex):
      pd.options.mode.chained_assignment = None
      a = int(startIndex[setNum])
      b = a + int(endIndex[setNum])
    ...
    #   subX['errF'] = self.erry.loc[setNum*itemsInSet:(setNum+1)*itemsInSet-1]
      subX = self.X.loc[a:b,:]
      subX['F'] = self.y.loc[a:b]
      subX['errF'] = self.erry.loc[a:b]
